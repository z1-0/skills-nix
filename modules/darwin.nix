    ...
    assertions = [
      {
        ...
        message = ''
          skills.dir "${cfg.dir}" uses '~' which is not expanded on nix-darwin.
          Use an absolute path instead, e.g.: skills.dir = "/Users/alice/.agents/skills"
        '';
      }
      {
        assertion = lib.all (t: !hasTilde t) cfg.symlink.targets;
        message = ''
          skills.symlink.targets contains '~' paths which are not expanded on nix-darwin.
          Use absolute paths instead, e.g.: skills.symlink.targets = [ "/Users/alice/.cursor/skills" ];
        '';
      }
    ];
